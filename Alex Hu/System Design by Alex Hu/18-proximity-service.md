...
![Image represents a search result page, likely from a location-based service like Google Maps or Yelp.  The top displays ](./images/4a12e044_figure-1-neary-search-on-yelp-PIX55ZBV.png)
...
![Image represents a relational database table schema named 'business'.  The table is depicted as a rectangular grid with ](./images/24262506_table-3-business-table-72LHQZZP.png)
...
![Image represents a system architecture diagram for a business service.  A user initiates requests through a load balance](./images/78b0aafe_figure-2-high-level-design-OLWE4F2G.svg)
...
![Image represents a map section centered on a point labeled 'me,' showing a portion of a city grid with streets and build](./images/f8011705_figure-3-two-dimensional-search-UCK2KWKU.png)
...
![Image represents a world map displaying two datasets overlaid on a geographical basemap.  The basemap shows continents (](./images/2e852aad_figure-4-intersect-two-datasets-XGXW3FF3.png)
...
![Image represents a hierarchical tree structure illustrating different indexing techniques for spatial data.  The root no](./images/1deef948_figure-5-different-types-of-geospatial-indexes-FFADHPSV.svg)
...
![Image represents a global map using a Geographic Coordinate System based on the WGS84 Datum.  The map is displayed in a ](./images/3336dc3a_figure-6-global-map-NWAMEPOX.png)
...
![Image represents a world map divided into four quadrants by two perpendicular lines intersecting at the center.  Each qu](./images/fc34919a_figure-7-geohash-XY4L6XHV.png)
...
![Image represents a hierarchical quadtree-like structure visualizing a world map partitioned recursively.  The top level ](./images/87e52b20_figure-8-divide-grid-Z5AJ3TFS.png)
...
![Image represents a geographical area, likely a section of a city, overlaid with a grid system.  The base map shows stree](./images/96b1d4d9_figure-9-shared-prefix-K37LAGTS.png)
...
![Image represents a geographical map showing two locations, La Roche-Chalais (labeled A) and Pomerol (labeled B), in Fran](./images/85592bbf_figure-10-no-shared-prefix-MFLTKKDZ.png)
...
![Image represents a map section overlaid with a grid, highlighting specific zones.  The base map shows a city block layou](./images/7946e5a0_figure-11-boundary-issue-XJWRE2EX.png)
...
![Image represents a sequential process demonstrating the expansion of a search area.  The image is composed of three iden](./images/21b36144_figure-12-expand-the-search-process-ZN5TJYNX.png)
...
![Image represents a system illustrating data flow through different stages.  The diagram consists of three rectangular bo](./images/43e92bde_figure-13-quadtree-MWKA5XCK.png)
...
![Image represents a tree-like structure, likely a quadtree, illustrating a hierarchical data partitioning scheme.  The ro](./images/ee2aa149_figure-14-build-quadtree-ZV3JS35E.svg)
...
![Image represents a geographical map, likely of the Denver, Colorado area, overlaid with a grid of large, bright green re](./images/341be8ea_figure-15-real-world-example-of-a-quadtree-WENVCDJ2.png)
...
![Image represents a visualization likely depicting a concept related to pathfinding or network routing within a constrain](./images/7453e37a_figure-16-hilbert-curve-A42ZBOV2.png)
...
- S2 is great for geofencing because it can cover arbitrary areas with varying levels (Figure 17). According to Wikipedia, “A geofence is a virtual perimeter for a real-world geographic area. A geo-fence could be dynamically generated—as in a radius around a point location, or a geo-fence can be a predefined set of boundaries (such as school zones or neighborhood boundaries)” [25].
  ...
  ![Image represents a map depicting a geofence overlaid on a street map.  The base map shows a section of a city with stree](./images/2d9fbb69_figure-17-geofence-PIWNFXYE.png)
...
- Updating the index is easy. For example, to remove a business from the index, we just need to remove it from the corresponding row with the same *geohash* and *business_id*. See Figure 18 for a concrete example.
  ...
  ![Image represents a simple table with two columns: 'geohash' and 'business_id'.  The 'geohash' column contains three entr](./images/640320a0_figure-18-remove-a-business-ELAJOMXR.png)
...
![Image represents a tree-like structure, likely a quadtree or similar spatial partitioning data structure.  The root node](./images/63d15e23_figure-19-update-quadtree-OCBKLCXE.svg)
...
![Image represents a table depicting a geospatial index. The table is bordered with a gray shadow effect.  The top row, sh](./images/1c1df861_table-9-list_of_business_ids-is-a-json-array-HUAEOYZJ.png)
...
![Image represents a table structure labeled 'geospatial_index'.  This table has two columns. The top row acts as a header](./images/4c1e553a_table-10-business_id-is-a-single-id-3ZNAELVH.png)
...
- Privacy laws. Some countries may require user data to be used and stored locally. In this case, we could set up a region in that country and employ DNS routing to restrict all requests from the country to only that region.
  ...
  ![Image represents a world map depicting the geographical distribution of server regions and user locations, illustrating ](./images/b873e943_figure-20-deploy-lbs-closer-to-the-user-OVZHVKZ6.png)
...
![Image represents a system architecture diagram for a business search service.  At the top, a Load Balancer (1) distribut](./images/6cbf1f70_figure-21-design-diagram-ORKTOGMW.png)
...
![Image represents a mind map outlining the design process for a Proximity Service.  The central node is 'Proximity Servic](./images/8190acaa_chapter-summary-CNKPFTKZ.png)
