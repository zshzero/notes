...
function createMockRuntime() {
  ...
  function jsx(type, props) {
    ...
    switch (tag) {
      ...
      case 'code': {
        // Always extract language from className (code runs before pre in JSX eval order)
        const cls = props?.className || '';
        const langMatch = cls.match(/language-(\w+)/);
        if (langMatch) currentCodeLang = langMatch[1];
      ...
      // Coding Patterns course components
      case 'CodeTabs': return { __md: children };
      case 'ProblemDescription': return { __md: children };
      case 'OpenCodeEditor': return { __md: '' };
      default: {
        if (props?.src) {
          const src = props.src;
          const alt = (props?.alt || 'image').substring(0, 120);
...
function codeToMarkdown(code, title) {
  ...
  try {
    ...
    if (mod && mod.default) {
      ...
      components.ProblemDescription = 'ProblemDescription';
      components.OpenCodeEditor = 'OpenCodeEditor';
