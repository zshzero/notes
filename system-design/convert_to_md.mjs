/**
 * Convert ByteByteGo course JSON files to Markdown.
...
function createMockRuntime() {
  ...
  function renderChildren(children) {
    ...
    // Fix: check 'in' instead of truthiness so empty string __md works
  ...
  function jsx(type, props) {
    ...
    switch (tag) {
      ...
      case 'code': {
        // Inside pre, don't add backtick wrapping (pre already adds ```)
      ...
      case 'pre': {
        ...
        insidePre = false;
        // Strip single backtick wrapping added by code element (evaluated before pre)
      ...
      case 'tr': {
        ...
        // Render each cell, flatten to single line, preserve empty cells
      ...
      case 'mfrac':
      case 'mtext':
        // Skip mrow/mn/mo etc. - annotation has the readable LaTeX source
        return { __md: '' };
      case 'annotation':
        return { __md: children };
      // Custom components
  ...
  /** Flatten table cell text to a single line. */
...
function codeToMarkdown(code, title) {
  ...
  try {
    ...
    if (mod && mod.default) {
      ...
      md = md.replace(/\n{3,}/g, '\n\n');
      md = md.replace(/\n#{1,6}\s*\n/g, '\n');
      // Clean up KaTeX artifacts: collapse $...$$ or empty $ pairs
      md = md.replace(/\$\s*\$/g, '');
